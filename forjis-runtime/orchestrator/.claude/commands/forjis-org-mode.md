...
3. For each `.md` file:
   ...
   e. **Files-touched oid compare** (only when frontmatter has a non-empty `files_touched` list; skip this sub-step when the key is absent or its value is `[]` — legacy tolerance per spec R2). For each entry `{path, oid}` in the list, in declaration order:
      - Run `git hash-object <path>` via the Bash tool with a 10000 ms timeout.
      - If the command fails because the path does not exist: record `cache_decision = "miss"`, `cache_reason = "file <path> no longer exists"`; emit `[orchestrator]: exploration cache — miss (file <path> no longer exists)` and STOP iterating the rest of this entry's `files_touched` list (but continue scanning remaining cache files in the outer loop).
      - If the command returns an oid that differs from the cached `oid`: record `cache_decision = "miss"`, `cache_reason = "file <path> changed since cache"`; emit `[orchestrator]: exploration cache — miss (file <path> changed since cache)` and STOP iterating the rest of this entry's `files_touched` list (again, continue the outer loop).
      - If every `{path, oid}` pair matches its current on-disk oid: treat the file as admissible — continue to step 4 (record hit).
...
5. Compose agent invocation:
   ...
   - **Visuals section (when present):** If the facilitator wrote `.forjis/tasks/<TASK_ID>/visuals-<org>-<team>-<role>.yaml` for this role, prepend a `## Visuals` section at the very top of the agent prompt (before the base agent body). Render the section per the layout in `openspec/changes/role-visuals/specs/requirements/spec.md` §"Orchestrator renders the `## Visuals` section from the artefact": (a) an intro paragraph telling the agent to use `Read` for static files, `Bash` + `curl` for URLs, and the listed capture tool via `Bash` for screenshots; (b) a "Pinned by user on this task:" list of every `pinAssets` entry when non-empty; (c) per-entry groupings — "Live URL — reachable:" for `http(s)` entries with `status: reachable`, the same URL with a `(probe failed)` suffix when `status: probe-failed`; (d) `Capture tool available: <name> (installed at <path>)` plus an example `cd <projectRoot>/<path> && npx <name> screenshot …` hint for each entry with a declared `tool`; (e) `Credentials file: <path>` plus a one-line reminder that the agent MUST read the file with `Read` and MUST NOT echo its contents to stdout, logs, or review notes for each entry with a declared `credentials` (omit this line when `credentialsMissing: true`); (f) `file://` / `dir://` group headers with each resolved absolute path as a bullet, falling back to `(no matches)` when `paths` is empty. When the artefact file does not exist, emit no `## Visuals` section and proceed as today.
   - **Codebase index section (Explorer / Analyst / Architect / Developer only — NOT Reviewer):** If the facilitator wrote `.forjis/tasks/<TASK_ID>/context-<org>-<team>-<role>.yaml` for this role, parse the YAML (fields `version`, `index_md`, `ranked_rows`, `prior_explorations`). Emit a `## Codebase index` section placed **after** the Visuals section and **before** the task-rule `prompt_prepend` bullet below. Section body:
     (a) the verbatim `index_md` string (a short orient doc produced by `forjis context refresh`);
     (b) a "Top N files ranked for this task:" list — one bullet per `ranked_rows` entry in the form `- <path> — <summary>`;
     (c) when the role is Analyst OR Architect AND `prior_explorations` is non-empty: append a `## Prior explorations` sub-section. For each entry, render its `body` headed by `### Task <taskId>`; append ` (current task)` to the heading when `isCurrentTask: true`. For Explorer and Developer, omit this sub-section even when `prior_explorations` is non-empty.
     When the artefact file is missing, cannot be parsed as YAML, or has an unexpected shape, emit no `## Codebase index` section and emit a single `[context-cache]: skipped — <reason>` log line. Reviewer roles always skip this bullet (no artefact file exists for them by construction — see spec R8).
